...
whitespaceBreak: false
    ...
    fs.existsSync('./data/src/whatsappWeb.json') && ALDI.loadAuthInfo('./data/src/whatsappWeb.json')
    ALDI.on('connecting', () => {
        start('2', 'Connecting...')
    })
    ALDI.on('open', () => {
    ...
    await ALDI.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./data/src/whatsappWeb.json', JSON.stringify(ALDI.base64EncodedAuthInfo(), null, '\t'))
...
if (!welkom.includes(anu.jid)) return
       ...
       let v = ALDI.contacts[num] || { notify: num.replace(/@.+/, "") };
        anu_user = v.user.name || v.vname || v.notify || v.name
